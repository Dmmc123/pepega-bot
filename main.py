...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if res:
        await context.bot.send_message(
            ...
            text=f"You already received your free tokens!\n"
                 f"So no more free tokens for you lol\n"
...
async def balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text=f"You have {tokens} tokens left!"
    )
...
def connect_init_db():
    ...
    client = MongoClient(f"mongodb://{os.environ['MONGO_INITDB_ROOT_USERNAME']}:{os.environ['MONGO_INITDB_ROOT_PASSWORD']}@db", 27017)
    ...
    if os.environ["db-collection"] not in db.list_collection_names():
        ...
        logging.log(
            level=logging.INFO,
            msg=f"Created collection {os.environ['db-collection']}!"
        )
    return client
...
if __name__ == '__main__':
    ...
    logging.log(
        level=logging.INFO,
        msg="Instantiated Answer Generator"
    )
